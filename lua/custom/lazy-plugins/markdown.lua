return {
    ...
    ---------------------------------------------------------------------------
    {
        ...
        keys = {
            ...
            },
            {
                ...
                function()
                    vim.cmd("edit")
                end, -- reliable re-render trigger
        ...
        config = function(_, opts)
            ...
            local function detect_backend()
                ...
                if
                    os.getenv("TERM")
                    and (string.match(os.getenv("TERM"), "sixel") or string.match(os.getenv("TERM"), "mlterm"))
                then
                    return "sixel"
                end
                return nil
            ...
            local function diagram_check(silent)
                ...
                if #missing == 0 then
                    if not silent then
                        vim.notify("diagram.nvim: all good ✅", vim.log.levels.INFO)
                    end
                    return true
                else
                    local msg = "diagram.nvim: missing tools → \n• " .. table.concat(missing, "\n• ")
                    if not silent then
                        vim.notify(msg, vim.log.levels.WARN, { title = "DiagramCheck" })
                    end
                    return false
                end
            end
            ...
            vim.api.nvim_create_autocmd("FileType", {
                pattern = "markdown",
                callback = function()
                    if vim.g.__diagram_checked_once then
                        return
                    end
    ...
    -- Web preview in your browser (Mermaid + TeX; requires Deno)
    {
        ...
        config = function()
            ...
            vim.keymap.set("n", "<leader>mwo", function()
                peek.open()
            end, { desc = "Web Preview: Open (Peek)" })
            vim.keymap.set("n", "<leader>mwc", function()
                peek.close()
            end, { desc = "Web Preview: Close (Peek)" })
